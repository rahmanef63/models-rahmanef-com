...
>
> Status: **v0.1 shipped** (Phases 1–5). This doc is now the DESIGN reference (data model §3, per-pillar detail §6, steal-from §7, non-goals §8). The per-item build log + verified-against-code scope + v0.2 backlog live in **`docs/FEATURES-LOG.md`**. Companion docs: `docs/AI-SLICES-PROGRESS.md`, `docs/COMPARISON-hermes-openclaw-9router.md`.
...
> **Status (2026-07-05):** Phases 1–5 shipped as a working **v0.1**. The full per-item build
> checklist + verified-against-code scope moved to **`FEATURES-LOG.md`**. This section now keeps
> only each phase's Definition of Done + the open v0.2 backlog pointer. Deferred wave-2 = Phase 6 / §8.

**Phase 1 — Workspaces** (`workspaces`, `byok`) — DoD: every public fn workspace-scoped; a second user is invited by link, shares a provider key under credPolicy, both burn tokens, removal instantly revokes all surfaces. → ✅ shipped. Open: 1.7 shared-cred WRITE.

**Phase 2 — Model gateway + memory** (`api-compat`, `provider-pool`, `combos`, `mcp-client`, `memory`) — DoD: Claude Code / Cursor point at `/v1` with an `sk-rr-` key and complete a real tool-call loop; agents consume external MCP; agents remember across sessions. → ✅ shipped **except 2.6 `/v1` tool passthrough** (open — external tool-loops don't work yet). Open: 2.4 rotation, 2.9 agent/summary injection.

**Phase 3 — Channels** (`channels` — Telegram/Slack/WhatsApp/Discord) — DoD: paste a bot token, DM the bot, bound agent replies on workspace creds; dedupe never double-answers. → ✅ shipped (Telegram/Slack/WhatsApp). Open: **3.10 Discord deferred-ack** (fails on realistic latency).

**Phase 4 — Scheduled agents** (`scheduled-agents`) — DoD: a scheduled agent runs unattended, its run shows in the trace viewer, result delivered to a peer, auto-pauses after 5 errors. → 🟡 shipped v0.1 (5-min sweep, interval-only). Open: 4.2 trace + auto-pause, 4.3 deliver/cadence UI.

**Phase 5 — Workspace ops** (`usage-rollups`, `audit-log`, spend-caps) — DoD: admin sees per-member/per-model estimated cost, sets a monthly USD cap that blocks new calls, reads a 90-day audit trail. → 🟡 shipped; spend-cap `runAgent` bypass fixed 2026-07-05. Open: 5.4 per-member view, 5.5 remaining audit sites.

**Open v0.2 backlog:** `FEATURES-LOG.md` → "v0.2 backlog" (11 verified gaps).

...
export const TOOL_REGISTRY: readonly ToolMeta[] = [
  ...
  { id: "connect_custom_provider", label: "Connect a custom provider",
    description: "Connect a custom OpenAI-compatible endpoint. Pick a name (becomes the provider slug), a baseURL (must speak /chat/completions), and an API key. Target models as '<name>/<model>'. Private/loopback/metadata URLs are blocked.",
    inputSchema: { type: "object", properties: { name: { type: "string" }, baseURL: { type: "string" }, apiKey: { type: "string" } }, required: ["name", "baseURL", "apiKey"], additionalProperties: false },
    surfaces: ["agent", "mcp"] },

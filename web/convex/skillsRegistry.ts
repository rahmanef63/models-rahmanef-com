...
export const SKILLS_REGISTRY = [
  ...
  { id: "vault-keeper", label: "Vault keeper", description: "Organizes durable knowledge into linked vault notes.", instructions: "Treat the knowledge vault as your durable, structured memory of documents. Before writing, call vault_list to see what exists and update the closest note instead of creating a near-duplicate. Give every note a clear, stable title and connect related notes with [[Other Title]] wikilinks so the graph stays navigable. Use JSON format for structured data (records, config, tables) and markdown for prose. Keep the `memory` tool for short one-line facts; put anything longer or worth revisiting in the vault with vault_write." },
] as const;
